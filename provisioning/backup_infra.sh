...
# --- 1. volumes Docker -------------------------------------------------------
# ciso_* exclus : CISO-Assistant est un produit SÉPARÉ de cette plateforme
# (aucune donnée échangée, public et rétention différents d'un labo CTI) —
# sa sauvegarde relève de sa propre politique, pas de celle-ci.
log "volumes Docker"
docker volume ls --format '{{.Name}}' | grep -vE '^ciso_' | while read -r v; do
...
for f in "${REPO_PLATFORM}/vendor/misp-docker/.env" "${REPO_PLATFORM}/opencti/.env" ${REPO_FARM:+"${REPO_FARM}/.env"}; do
...
log "manifeste"
{
  ...
  echo "- config/      : fichiers .env (SECRETS : clés MISP, jeton OpenCTI), crontab, images à retélécharger"
  echo "                 (CISO-Assistant, produit séparé, N'EST PAS sauvegardée ici — voir"
  echo "                 ciso-assistant/, sa politique de rétention n'est pas celle d'un labo CTI)"
  ...
  echo
  echo "## Restauration"
  echo "1. détarer les dépôts, remettre les .env dans vendor/misp-docker/ et opencti/ (et celui du dépôt d'alimentation s'il était sauvegardé)"
